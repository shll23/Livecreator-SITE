    ...
    <section id="profile-showcase" className="py-10 sm:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-12 gap-3 sm:gap-10 items-start max-w-6xl mx-auto">
          {/* Bild LINKS — 2 Bilder gestapelt damit Höhe zum Text passt */}
          <div className="col-span-5">
            <div className="flex flex-col gap-2 sm:gap-3">
              {/* Bild 1 — Haupt */}
              <div className="relative aspect-[4/5] rounded-lg sm:rounded-3xl overflow-hidden shadow-lg sm:shadow-xl">
                <img src="/profiles/frau-2-valentina/01-haupt.jpg" alt={FEATURED.name} className="w-full h-full object-cover" />
...
function StickyCTA() {
  ...
  return (
    <>
      ...
      {/* Mobile — rechts angedockt, kompakt */}
      <div
        className={`sm:hidden fixed right-3 z-40 transition-all duration-500 ${
          shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 pointer-events-none'
        }`}
        style={{ bottom: '5rem', maxWidth: '75%' }}
      >
        <Link
          href="/register"
          className="group flex items-center gap-2.5 bg-brand-600 text-white font-semibold pl-2 pr-4 py-2.5 rounded-full shadow-[0_8px_32px_-4px_rgba(236,72,153,0.6)] hover:bg-brand-700 active:scale-95 transition-all"
        >
          ...
          <span className="text-sm font-semibold whitespace-nowrap">Jetzt schreiben</span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="shrink-0 group-hover:translate-x-1 transition-transform">
            <path d="M5 12h14M13 5l7 7-7 7" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </Link>
      </div>
