...
// Mutex: stellt sicher dass nur EIN refresh-Call gleichzeitig laeuft.
...
export interface MonthlyEarnings {
  ...
  existing_payout_id: string | null;
}
...
export async function adminListPendingPhotos(): Promise<{ photos: PendingPhoto[] }> {
  return api('/api/admin/photos/pending');
}

export async function adminApprovePhoto(id: string): Promise<{ approved: boolean }> {
  return api(`/api/admin/photos/${id}/approve`, { method: 'POST' });
}

export async function adminRejectPhoto(id: string, reason: string): Promise<{ rejected: boolean }> {
  return api(`/api/admin/photos/${id}/reject`, {
    method: 'POST',
    body: JSON.stringify({ reason }),
  });
}

export async function adminListCreators(): Promise<{ creators: AdminCreator[] }> {
  return api('/api/admin/creators');
}

export async function adminGetMonthlyEarnings(creatorID: string, year: number, month: number): Promise<MonthlyEarnings> {
  return api(`/api/admin/creators/${creatorID}/monthly-earnings?year=${year}&month=${month}`);
}

export async function adminListAllPayouts(): Promise<{ payouts: AdminPayout[] }> {
  return api('/api/admin/payouts');
}
...
export async function adminUploadInvoice(payoutID: string, file: File): Promise<{ uploaded: boolean }> {
  const token = getAccessToken();
  if (!token) throw new APIError(401, 'unauthenticated');
  ...
  const base = process.env.NEXT_PUBLIC_API_URL || '';
  const res = await fetch(`${base}/api/admin/payouts/${payoutID}/invoice`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    body: formData,
  });
...
// ============================================================================
// PHASE G1+G2: Plattform-Stats, Kunden, Coin-Kaeufe
// ============================================================================
...
export interface PlatformStats {
  ...
  coins_sold_total: number;
  coins_sold_month: number;
  customers_total: number;
  customers_active_month: number;
...
export async function adminPlatformStats(): Promise<PlatformStats> {
  return api('/api/admin/stats/platform');
}
...
export async function adminListCustomers(): Promise<{ customers: AdminCustomer[] }> {
